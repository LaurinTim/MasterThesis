...
LYADFILE = Path('/eos') / 'experiment' / 'gbar' / 'datasummary'
...
def LyA_addDate(date, corr_df = pd.read_csv('/eos/user/l/lkoller/SWAN_projects/data analysis 2024/LyA/special stuff/correction_arrays.csv').fillna(-100)):
    ...
    trc = trc[[True if val in list(summary.LyA) else False for val in list(trc.LyA)]]
    ...
    s1 = summary[[True if val in list(trc.LyA) else False for val in list(summary.LyA)]]
    ...
    i1 = s1.index[[str(val) != 'None' for val in s1['LyA']]] #indices in the summary that correspond to the rows in trc
    trc.set_index(i1, inplace = True) #set the indices of trc to i1
    
    s2 = summary[[True if val in list(wav.MCP5_Waveform) else False for val in list(summary.Waveform_12bit)]]
