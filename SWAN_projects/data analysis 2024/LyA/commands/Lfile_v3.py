...
class Lfile():
    ...
    def parpeak(self, ch, peak, edges = [0, 0]):
        ...
        '''
        #xdat = [self.ts_from_ms(val) for val in self.data[0][0]]
        xdat = self.data[0][0]
        ydat = self.data[1][ch]
    ...
    def parpeak2(self, ch, peak):
        ...
        while peak-lst1>=1 and lst1<20 and ydat[peak-lst1-1]>=0.003:# and not (lst>5 and ydat[peak-lst-2]>ydat[peak-lst-1] and ydat[peak-lst-3]>ydat[peak-lst-2]):
            lst1 += 1

        while peak+rst1<=10000 and rst1<20 and ydat[peak+rst1+1]>=0.003:# and not (rst>5 and ydat[peak+rst+2]>ydat[peak+rst+1] and ydat[peak+rst+3]>ydat[peak+rst+2]):
            rst1 += 1
        ...
        if args[0][0]<=args[0][1]:
            return [args[0][0], args[0][1], args[0][2], args[0][3]], diff
        
        else:
            return [args[0][1], args[0][0], args[0][3], args[0][2]], diff
    ...
    def get_events_from_file(self, hgt = 0.005, dist = 5, trange = None):
        ...
        lims = [800, 600, 500, 600] #until how many timesteps after the peak we have to correct the voltage (changes depending on the channel)
        ...
        for i in [0,1,2,3]:            
            ...
            while len(peaks[0]) > 0:
                ...
                while len(peaks[0]) > 0 and d0 > 0.003 and (diff2 > 0.001 or abs(args[0]-args[1]) < 1):
                    peaks[0] = peaks[0][1:]
            ...
            for k in range(len(curr_pt)):
                ...
                if curr_pt[k] != -1: t0, h0, d0 = self.parpeak(i, curr_pt[k])
                else: t0, h0, d0 = -1, 1, 1
