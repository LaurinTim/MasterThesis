...
class Lfile():
    ...
    def __init__(self, filepath, df = None, corr_df = None):
        ...
        if type(df) != type(None):
            ...
            self.df = pd.read_csv('/eos/experiment/gbar/datasummary/datafile24.txt', delimiter = '\t')
        
        #if 'LY_LED' in self.filepath or not self.filepath in self.df:
        #    self.valid = False
    ...
    def run_number(self):
        ...
        '''
        if not self.filepath in self.df: return 0
    ...
    def mw_on(self):
        ...
        1 if mw was on
        0 if mw was off
        -1 if mw in unknown (self.filepath not in self.df)
        
        '''
        if not self.filepath in self.df: return -1
    ...
    def get_events_from_file(self, hgt = 0.005, dist = 5, trange = None):
        ...
        '''
        if self.valid == False:
            mw_on_curr = self.mw_on()
            run_number_curr = self.run_number()
            
            return [[],[],[],[]], [[],[],[],[]], mw_on_curr, run_number_curr
        ...
        for i in [0, 1, 2, 3]:
            ...
            while len(peaks[0]) > 0:
                ...
                if d0 > 1e-4 and diff2 < 0.001 and (diff2 < d0/2 if h0 >= maxvol else diff2 < d0/2) and abs(args[0]-args[1]) >= 1 and pp[i][-1] <= 3:
