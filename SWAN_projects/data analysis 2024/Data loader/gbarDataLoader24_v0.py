...
sequence_logFile = ['Time', 'run number', 'event number', 'DAQ', 'pbar trap', 'positron', 'Valve in', 'MCP in', 'MW power', #8 general info
...
sequence_files = ['MCP1', 'MCP2', 'MCP3', 'MCP3.5', 'MCP4', 'MCP5', 'MCP7', 'Waveform_12bit', 'CMOS_Tracker', 'DRS4',
...
sequence_summary = ['Time','Datetime','run number','event number', 'DAQ', 'pbar trap', 'positron', 'MCP1', 'MCP2', 'MCP3', 'MCP3.5', 'MCP4', 'MCP5', 'MCP7', 'Waveform_12bit', 'CMOS_Tracker', 'DRS4',
                   ...
                   'Valve in', 'MCP in', 'MW power', 'H-sou pressure', 'cube pressure', 'RC pressure', 'MW pressure','SwY pressure','LyA pressure', 
...
def loadLog(date2an):
    ...
    if ((DATASUMMARY / date2an / ('log_'+date2an+'.txt')).is_file() == True) and (date2an < datetime.fromtimestamp((DATASUMMARY / date2an / ('log_'+date2an+'.txt')).stat().st_mtime).strftime('%y_%m_%d')):
        ...
        if not (PROTONGUNPC / date2an).exists() or not (PROTONGUNPC / date2an / (date2an+'.txt')).is_file():
            ...
            #we want to change the run number to 0 if an event does not belong to any run, since in the logfile these events have the run number of the previous run
            runf = loadRun() #info about the runs
            crunset = sorted(set(log['run number'])) #different run numbers in the log file
            runset = set(runf['run number']) #different run numbers in the run info
            
            #go through the run numbers in the log file
            for i in crunset:
                rnum = float(i)
                
                #if the log run number is not in the run info file, we set the run number to 0 (this should pretty much only happen for the 24_04_30)
                if not (rnum in runset):
                    log.loc[list(log.loc[:, 'run number'] == i), 'run number'] = 0
                    if date2an != '24_04_30': #if this happens for another date than '24_04_30', we want to know
                        print('error, run number from log file not in run info file')
                
                else:
                    #if run number + 1 is not in the log file, we set all run number after the current run to 0 (this should only happen for the last run of date2an)
                    ...
                    #set the run numbers in between the runs to 0
                    else:
...
def loadDatafile(date2an):
    ...
    if ((DATASUMMARY / date2an / ('summary_'+date2an+'.txt')).is_file() == True) and (date2an < datetime.fromtimestamp((DATASUMMARY / date2an / ('summary_'+date2an+'.txt')).stat().st_mtime).strftime('%y_%m_%d') ):
        ...
        df_final = pd.concat([strucdf,dflog]).reset_index(drop = True).drop([0])
